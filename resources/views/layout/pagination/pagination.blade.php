{{-- 件数表示（ユーザー側で使用）--}}
<div class="top_pagination" style="max-width: 100vw;">
    ...
    <div class="pagination" style="font-size: large; width: 100%; margin: 0 0 10px -2px;">
        ...
        @for($i = max($paginator->currentPage()-1, 1); $i <= min(max($paginator->currentPage()-1, 1)+2,$paginator->lastPage()); $i++)
            <span class="{{ ($paginator->currentPage() == $i) ? ' active' : '' }}" style="display: inline-block; text-align: center; border: 1px solid #333333;">
    ...
    @elseif ($paginator->lastPage() == 1)
        <span class="active" style="display: inline-block; text-align: center; border: 1px solid #333333;">
