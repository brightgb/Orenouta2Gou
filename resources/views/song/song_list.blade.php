...
<div class="absolute_part">
    ...
    <div class="cp_ipselect cp_sl01">
        <select onchange="location.href=value;">
            ...
            <option value="/song/song_list?sort=2" @if ($sort == 2) selected @endif>
                投稿の古い順
            </option>
            <option value="/song/song_list?sort=3" @if ($sort == 3) selected @endif>
                アドバイスの多い順
            </option>
            <option value="/song/song_list?sort=4" @if ($sort == 4) selected @endif>
                アドバイスの少ない順
            </option>
        </select>
    </div>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        <td style="width: 15%; padding-right: 1%; text-align: right;">
                            <img src="/storage/icon/song_item.png" style="height: 30px;">
                        </td>
                        <td class="detail_text">
                            <a href="/song/song_detail/{{ $item['song_id'] }}" style="font-weight: bold; text-decoration: none; color: blue;">
                                {{ $item['song_title'] }}
                            </a>
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 15%; padding-right: 1%; text-align: right; vertical-align: top;" rowspan="2">
                            <img src="/storage/icon/admin_comment.png" style="height: 30px;">
                            @if (!empty($item['fav_flg']))
                                <br>
                                <img src="/storage/icon/fav_flg.png" style="height: 20px; position: relative; left: -7px;">
                            @endif
                        </td>
                        <td class="detail_text">
                            {{ $item['nickname'] }}
